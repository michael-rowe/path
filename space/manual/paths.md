...
1. Click **+ Capture** (or press `Ctrl-Alt-c`)
2. Pick **Path**
3. Replace the suggested page name with a short slug for the Path (e.g. `paths/sfhea` or `paths/hcpc-renewal-2027`)
4. Fill in the page attributes in the Inspector (right-hand panel): framework, status, target date
5. The criteria for the framework are installed alongside the Path. The pre-installed UoL Professor path is a worked example you can browse.
